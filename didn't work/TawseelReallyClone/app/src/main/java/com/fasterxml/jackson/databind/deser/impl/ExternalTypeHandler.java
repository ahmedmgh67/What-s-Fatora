    ...
    throws IOException
  {
    ...
    while (i < j)
    {
      ...
      if (str == null)
      {
        ...
        else
        {
          if (!this._properties[i].hasDefaultType()) {
            throw paramDeserializationContext.mappingException("Missing external type id property '%s'", new Object[] { this._properties[i].getTypePropertyName() });
          }
      ...
      else
      {
        ...
        throw paramDeserializationContext.mappingException("Missing property '%s' for external type id '%s'", new Object[] { this._properties[i].getProperty().getName(), this._properties[i].getTypePropertyName() });
      }
    }
    i = 0;
    while (i < j)
...
/* Location:              H:\As A Bussines Man\confedince\App Dev Department\What's Fatora\Tawseel APK\Client\dex2jar-2.0\t-dex2jar.jar!\com\fasterxml\jackson\databind\deser\impl\ExternalTypeHandler.class
